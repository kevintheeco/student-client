...
class TestLoadGuardrails:
    def test_loads_claude_md_full_and_docs_as_paths(self, executor, tmp_project):
        with patch.object(ex, "ROOT", tmp_project):
            result = executor._load_guardrails()
        # CLAUDE.md는 전문 주입
        assert "# Rules" in result
        assert "rule one" in result
        # docs는 경로 목록만 주입 (내용은 on-demand)
...
class TestCheckoutBranch:
    ...
    def test_branch_exists_checkout(self, executor):
        # rev-parse HEAD → fetch → checkout 성공
        self._mock_git(executor, [
    ...
    def test_branch_not_exists_create(self, executor):
        # rev-parse HEAD → fetch → checkout 실패 → checkout -b 성공
        self._mock_git(executor, [
            MagicMock(returncode=0, stdout="main\n", stderr=""),
            MagicMock(returncode=0, stdout="", stderr=""),
    ...
    def test_checkout_fails_exits(self, executor):
        # rev-parse HEAD → fetch → checkout 실패 → checkout -b 도 실패
        self._mock_git(executor, [
            MagicMock(returncode=0, stdout="main\n", stderr=""),
            MagicMock(returncode=0, stdout="", stderr=""),
    ...
    def test_default_branch_is_feat_phase(self, executor):
        assert executor._branch == "feat-mvp"

    def test_custom_branch_from_index(self, tmp_project, phase_dir):
        # index.json에 branch 필드가 있으면 feat-{phase} 대신 그 브랜치를 쓴다 (기획자 브랜치 인수)
        index = json.loads((phase_dir / "index.json").read_text())
        index["branch"] = "planner-vibe"
        (phase_dir / "index.json").write_text(json.dumps(index, ensure_ascii=False))

        with patch.object(ex, "ROOT", tmp_project):
            inst = ex.StepExecutor("0-mvp")
        assert inst._branch == "planner-vibe"
        ...
        assert ("checkout", "planner-vibe") in calls
