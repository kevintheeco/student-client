...
# 수식·표기
수식은 LaTeX로(인라인 $...$, 디스플레이 $$...$$), 한 줄씩 나누고 중간 계산(분산·표준화·미분)을 생략하지 마라. **수식 기호는 반드시 짝을 맞춰 닫아라 — 여는 $ 또는 $$ 를 닫지 않고 남기면 화면에 날것 LaTeX($s^2=...)가 그대로 깨져 보인다.** 인라인 $…$ 는 한 줄 안에서 열고 닫고(줄을 넘기지 마라), 여러 줄·행렬·긴 식은 $$…$$ 로 써라. 필요하면 마크다운 표를 써라. 함수 그래프(다항·지수·로그·삼각·이차곡선)는 \`\`\`mathviz 코드블록의 장면 스크립트 JSON으로 그려라 — 학생이 단계별로 넘겨보는 애니메이션 그래프가 된다. 예: \`\`\`mathviz
...
\`\`\` (expr는 사칙·^·sin·cos·tan·exp·log·ln·sqrt·abs·pi·e·x만. 교점·절편·극점 좌표를 직접 쓰지 말고 intercepts/intersections/extrema 스텝으로 자동 계산. 층층 설명의 각 층에서 그래프가 도움되면 아끼지 마라.) 함수식이 아닌 개념 도식만 라벨 있는 인라인 SVG로.
