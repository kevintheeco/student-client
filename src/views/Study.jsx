...
function Study({deck:initial,subjects,onExit}){
  ...
  async function next(d,forceId,practical){
    ...
    if(examPool.length){
      ...
      if(box>=3){
        ...
        const sys=deep
          ?(d.k12
            ?"너는 심화문제 출제 전문가야. 아래 [원본 기출문제]가 다루는 개념을 더 깊이 파고드는 심화 계산 문제 하나를 만들어. 원본보다 어렵게: 조건을 추가하거나 여러 단원 개념을 엮되, 반드시 답이 명확한 값·식으로 떨어지는 실전형(내신 심화·수능 유형)으로. '~을 설명하라' 식 서술 요구 금지. 수식은 LaTeX($...$). 반드시 형식으로만 출력:\nQTYPE: apply\nQUESTION: 질문 내용\nPOINTS: 핵심1 | 핵심2 | 핵심3"
    ...
    // ── 중·고등 수학 덱(k12): 실전 문제풀이형 출제 — 설명·증명 요구는 출제 단계에서 금지 ──
    const isK12=!!d.k12;
    const qtypeRule=practical
      ?"▶ QTYPE: apply 고정 — 수치나 조건을 직접 주고 계산·풀이를 요구하는 문제."
      :isK12
      ?(box<=2
        ?"▶ QTYPE: recall 또는 apply — 공식·정의·값을 정확히 쓰는 단답형이거나, 그 공식을 바로 대입하는 기본 계산 문제. (예: '이차방정식의 근의 공식을 써라', '$x^2-5x+6=0$의 두 근을 구하시오')"
        :box<=3
        ?"▶ QTYPE: apply — 교과서 예제~내신 기본 수준의 계산 문제. 조건(수치·식·도형 정보)을 직접 주고 값·식을 구하게 하라."
        :"▶ QTYPE: apply — 내신 심화~수능 수준. 두 개 이상의 조건이나 개념을 엮은 응용 계산 문제. 답이 명확한 수치·식으로 떨어져야 한다.")
      :box<=2
    ...
    try{
      const sysK12=
        (d.k12us
          ?"너는 미국 중·고등학교 수학(Common Core·SAT·AP) 실전 문제 출제위원이야."
          :"너는 대한민국 중·고등학교 수학 내신·수능 출제위원이야.")+
        " 아래 자료의 목표 개념으로, 학생이 시험장에서 실제로 만나는 '문제' 하나를 만들어.\n\n"+
        "━━ 실전형 원칙 (가장 중요) ━━\n"+
        "✓ 문제는 반드시 구체적 조건(수·식·도형 정보)을 주고, 답이 명확한 값·식·결론으로 떨어지게 하라.\n"+
        "✓ 실제 내신·수능(또는 SAT) 기출 유형을 따르라 — 교과서·문제집에 실제로 있는 형태.\n"+
        "✗ '~을 설명하라', '~을 서술하시오', '왜 그런지 논하라' 식 백지 증명·서술 요구는 절대 금지. (증명이 교육과정 핵심인 소수 단원도 '다음 빈칸을 채워 증명을 완성하라'처럼 구조를 준다.)\n"+
        "✗ 정의를 그대로 받아쓰게 하는 문제 금지 (recall도 '공식·값을 쓰라'까지만).\n"+
        "✗ 자료에 근거 없는 내용, 교육과정 밖 내용 금지.\n\n"+
        qtypeRule+"\n\n"+
        "━━ 좋은 예 (이런 결의 문제) ━━\n"+
        "· '$\\triangle ABC$에서 $\\angle B=90°$, $\\overline{AB}=6$, $\\overline{BC}=8$일 때 외접원의 반지름을 구하시오.'\n"+
        "· '이차함수 $y=x^2-4x+k$의 그래프가 $x$축과 서로 다른 두 점에서 만나도록 하는 정수 $k$의 최댓값을 구하시오.'\n"+
        "· '두 수열의 합 $\\sum_{k=1}^{10}(2k+1)$의 값을 구하시오.'\n\n"+
        "POINTS(채점 핵심)는 '정답 값'과 '핵심 풀이 단계'로 채워라 (예: '외심은 빗변의 중점 | 빗변 길이 10 | 반지름 5').\n"+
  ...
  async function buildQuizItem(c,signal){
    ...
    const common=(deck.k12
      ?"너는 "+(deck.k12us?"미국":"대한민국")+" 중·고등학교 시험 퀴즈 출제 전문가야. 실제 내신 시험에 나오는 결로,"
      :"너는 대학 학습용 퀴즈 출제 전문가야.")+" 아래 자료 범위 안에서 목표 개념을 묻는 문제 하나를 만들어. 수식은 LaTeX($...$)로. 자료에 근거 없는 내용은 묻지 마.";
    let sys;
    if(fmt==="mc"){
  ...
  async function generateFollowup(ev0,history,myAnswer){
    ...
    try{
      ...
      const raw=await callAI(
        ...
        (deck.k12?"④-1 학습자는 중·고등학생이다. 추상적인 '왜'보다, 막힌 다음 풀이 단계를 향해 한 걸음 내딛게 하는 힌트형 질문으로 (예: '빗변이 지름이 되는 건 알았네 — 그럼 빗변 길이부터 구하면?').\n":"")+
  ...
  async function explainMore(){
    ...
    try{const r=await callAI(
      "너는 친절한 튜터야. 자료에 근거해 더 깊이 직관적으로(비유·예시·왜) 반말로 설명해. 수식·행렬·그리스 문자·수학 기호(⊥ ∥ ∈ ∉ ⊆ ℝ ∇ × · 등)는 반드시 모두 LaTeX로: 인라인은 $...$, 블록은 $$..$$. 유니코드 기호 직접 사용 금지. 그래프·곡선·다이어그램이 이해에 도움이 되면 적극 그려줘 — 함수 그래프는 아래 형식 규칙 4의 mathviz 장면 스크립트로(교점·극점 자동 계산), 함수식이 아닌 개념 도식만 인라인 SVG(코드블록 없이 <svg> 직접, width≤380 height≤260 viewBox, 축·레이블 포함, font-size 11~13, 축선=#221C39, 곡선 #6C5CE7·#27C2A0·#FF6B8A·#FFC24B, 배경=#FFFDF8)."+(examSol?" 아래 [기출 공식 해답]이 주어졌어 — 그 해답의 풀이 기조·논리 전개 순서·용어·표기를 그대로 따라가되, 학습자가 스스로 이해하도록 각 단계를 더 자세히 풀어서 설명해. 공식 해답과 어긋나는 설명은 하지 마.":"")+RICH_FMT+"\n\n자료:\n"+studyMat,
