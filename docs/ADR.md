...
### ADR-012: Next.js 전환 (2026-07-07) — ADR-011 폐기 — **폐기: ADR-013으로 대체**
...
### ADR-013: React SPA(Vite) 유지 확정 — Next.js 전환 철회 (2026-07-08) — ADR-012 폐기
**결정**: Next.js 전환을 철회하고 Vite + React 18 SPA를 유지한다. ADR-011의 결정을 복원·확정한다. 서버 실행이 필요한 기능은 기존 Cloudflare Worker(`proxy/worker.js`)를 확장해 담당한다.
**이유**: ADR-012의 동기(분석·기록 화면의 서버 렌더링)를 코드 수준에서 재검토한 결과, 실제 격차는 프레임워크가 아니라 데이터·인증의 서버 접근성이었다 — 시험 기록(`ng:exam:*`)·기출은행은 기기 IndexedDB에만 있어 동기화에서 제외되고(`src/core/platform.js`의 `noSync`), 인증은 클라이언트 Firebase Auth뿐이라 서버가 요청자를 식별할 수 없다. 이 전제가 해결돼도 해당 화면은 로그인 뒤 대시보드라 SEO가 불필요하고, 클라이언트 렌더로 충분하다. 반면 전환 비용은 큼: 해시 라우팅(`#student`/`#academy`)은 서버에 전달되지 않아 경로 라우팅 재설계 필요, CDN 전역 라이브러리(firebase-compat·KaTeX·pdf-lib)의 npm 재작업, 동기식 스토리지 캐시(`LS.get`)의 하이드레이션 불일치, 배포처 이전.
**트레이드오프**: 인증·레이트리밋·업로드 검증·마스킹 강제를 Worker에서 직접 구현해야 한다(프레임워크가 대신해주지 않음). 프론트/백이 리포 내 별도 조각(src/ vs proxy/)으로 유지된다.
**재검토 트리거**: (1) 검색 유입이 필요한 공개 콘텐츠 페이지가 로드맵에 오를 때, (2) Worker 엔드포인트가 5~6개 이상으로 커져 단일 리포·단일 배포의 이득이 실제로 생길 때.
**후속 조건 (이 결정의 전제)**: ① `ng:attempts` 단일 배열 → `users/{uid}/attempts/{id}` append-only 컬렉션 + 집계 문서 분리, ② `aca_shared` 업로드를 Worker 경유로 이전해 마스킹·필드 검증을 서버에서 강제, ③ 개인 BYOK 브라우저 직접 호출(`anthropic-dangerous-direct-browser-access`) 제거 — 전 경로 Worker 프록시화, ④ TypeScript + Vitest 점진 도입, worker.js 엔드포인트별 분리(ADR-011 승계).

---

### ADR-014: 로컬 데이터 스키마 버전 + 동기화 병합 규칙 명문화 (2026-07-08)
**결정**:
1. **스키마 버전(A1)**: 모든 `ng:*` 로컬 데이터의 형식 버전을 `ng:__schema`에 기록한다(현재 v1). 저장 형식을 바꾸는 변경은 `SCHEMA_VERSION`을 올리고 `platform.js`의 `_MIGRATIONS[새버전]`에 변환 함수를 추가한다. 앱 시작 시 낮은 버전은 순차 마이그레이션, 데이터가 앱보다 새 버전이면 마이그레이션·버전 스탬프를 중단하고 경고한다(구버전 앱이 신버전 데이터를 오염시키지 않게).
2. **병합 규칙(A2)**: 로컬↔Firestore 동기화는 **키 단위 last-write-wins**다 — 키별 수정시각(`ng:__meta`)과 클라우드 문서의 `t`를 비교해 최신이 이긴다(동률이면 로컬 유지+업로드). 두 기기에서 같은 키를 다르게 고치면 늦게 저장한 쪽이 이기고 다른 쪽 변경은 소실된다 — 이것은 알려진 트레이드오프다(CRDT 도입 안 함). 완화책: 클라우드가 로컬을 덮기 직전의 원본을 `ng:__syncBackup`(마지막 동기화 1회분)에 보존해 수동 복구 가능하게 한다.
3. `ng:__` 접두 키(내부 메타)는 동기화에서 제외한다.
**이유**: 로컬 우선 앱의 최대 리스크는 '구버전 데이터 기기 × 신버전 앱' 충돌 — 데이터가 적은 지금 버전 스탬프를 넣어야 나중에 소급 가능하다. 병합은 학습 데이터 특성상(한 사람이 한 기기씩 순차 사용) 키 단위 LWW로 충분하고, CRDT는 오버엔지니어링.
**트레이드오프**: 동시 편집 충돌은 감지하지 못하고 조용히 덮는다. 백업은 1회분만 보존.
**검증**: `src/core/__tests__/platform.test.js`(마이그레이션 러너), `srs.test.js`(스케줄 로직) — Vitest(`npm test`).
